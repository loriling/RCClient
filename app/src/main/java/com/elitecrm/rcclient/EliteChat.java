...
public class EliteChat {
    ...
    public static Activity getConversationActivity() {
        return conversationActivity;
    }

    public static void setConversationActivity(Activity conversationActivity) {
        EliteChat.conversationActivity = conversationActivity;
    }
     ...
     */
    public static void initAndStart(String serverAddr, String userId, String name, String portraitUri, String targetId, Context context, int queueId, String ngsAddr, String from, String tracks, String title) {
        ...
        if (client != null && client.getLoginId().equals(userId) && client.getName().equals(name) && client.getIcon().equals(portraitUri) &&
            ...
            chat.initClient(userId, name, portraitUri, targetId, title);
            chat.initRequest(queueId, from, tracks);
            //发出聊天排队请求
            chat.sendChatRequest();
            //启动聊天会话界面
            RongIM.getInstance().startConversation(EliteChat.context, Conversation.ConversationType.PRIVATE, targetId, getTitle(title));
        } else {
            ...
            new FetchTokenTask().execute(serverAddr, userId, name, portraitUri, targetId, title);
        }
    }
     ...
     * @param context
     */
    public static void switchToChat(Context context, String target, String title) {
        if (context != null) {
            ...
            builder.appendPath("conversation").appendPath(Conversation.ConversationType.PRIVATE.getName())
                    ...
                    .appendQueryParameter("title", getTitle(title));
    ...
    public static void checkToken(String serverAddr, String token,  String userId, String name, String portraitUri, String targetId, Context context, int queueId, String ngsAddr, String from, String tracks, String title) {
        EliteChat.context = context;
        new CheckStateTask().execute(serverAddr, token, userId, name, portraitUri, targetId, queueId + "", ngsAddr, from, tracks, title);
    }
        ...
        if (Chat.getInstance().isSessionAvailable()) {
            if (Chat.getInstance().isTokenValid()) {
                if (queueId != Chat.getInstance().getRequest().getQueueId()) {//如果队列改变了，则直接结束之前会话，并开启新的会话
                    EliteChat.closeSession(serverAddr, Chat.getInstance().getSession().getId(), userId, name, portraitUri, targetId, context, queueId, ngsAddr, from, tracks, title);
                } else {// 如果队列没变化，则去检查token是否还合法，如果合法则可以直接继续聊天。如果不合法则需要重新登录
                    EliteChat.checkToken(serverAddr, Chat.getInstance().getToken(), userId, name, portraitUri, targetId, context, queueId, ngsAddr, from, tracks, title);
                }
            } else {
                // 如果token不合法，则重新初始化
                EliteChat.initAndStart(serverAddr, userId, name, portraitUri, targetId, context, queueId, ngsAddr, from, tracks, title);
            }
        } else {
            // 如果会话已经不可用，则重新初始化
            EliteChat.initAndStart(serverAddr, userId, name, portraitUri, targetId, context, queueId, ngsAddr, from, tracks, title);
    ...
    public static class FetchTokenTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                Chat.getInstance().initClient(userId, name, portraitUri, targetId, title);
            ...
            RongIM.connect(token, new RongIMClient.ConnectCallback() {
                ...
                public void onSuccess(String userId) {
                    ...
                    if (startChatOnReady) {
                        ...
                        RongIM.getInstance().startConversation(EliteChat.context, Conversation.ConversationType.PRIVATE, Chat.getInstance().getClient().getTargetId(), getTitle(Chat.getInstance().getClient().getTitle()));
                    }
                }
    ...
    public static class CheckStateTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            if ("success".equals(result)) {
                RongIM.getInstance().startConversation(EliteChat.context, Conversation.ConversationType.PRIVATE, Chat.getInstance().getClient().getTargetId(), title);
            } else {
                Chat.getInstance().clear();
                initAndStart(serverAddr, userId, name, portraitUri, targetId, EliteChat.context, queueId, ngsAddr, from, tracks, title);
            }
        }
    }
    ...
    public static void closeSession(String serverAddr, long sessionId, String userId, String name, String portraitUri, String targetId, Context context, int queueId, String ngsAddr, String from, String tracks, String title) {
        EliteChat.context = context;
        new CloseSessionTask().execute(serverAddr, sessionId + "", userId, name, portraitUri, targetId, queueId + "", ngsAddr, from, tracks, title);
    }
